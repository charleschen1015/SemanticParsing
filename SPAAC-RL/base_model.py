...
class BaseModel(object):
    ...
    def beam_greedy(self, sess, cont1, cont2, cont3, initial_memory, initial_output, vocabulary):
        config = self.config
    ...
    def beam_sample(self, sess, cont1, cont2, cont3, initial_memory, initial_output, vocabulary):
        config = self.config
        ...
        for idx in range(config.max_output_length):
            ...
            for b in range(num_steps):
                ...
                memory, output, scores, scores2, scores3, sample_word = sess.run(
                    [self.memory, self.output, self.probs, self.probs2, self.probs3, self.sample_word],
                    feed_dict = {self.b_ctx1: cont1, self.b_ctx2: cont2, self.b_ctx3: cont3,
                                 self.last_word: last_word,
                                 self.last_memory: last_memory,
                                 self.last_output: last_output})#,
                ...
                for k in range(config.batch_size):
                    ...
                    samples = sample_word[k] #_samples.argsort()[-3:][::-1] #_samples
                    words_and_scores = [(_word, scores[k][_word]) for _word in samples]  # scores[k][_word]
